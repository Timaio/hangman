class Game
  # Количество допустимых ошибок.
  TOTAL_ERRORS_ALLOWED = 7

  # Конструктор класса Game на вход получает строку с загаданным словом.
  # В конструкторе инициализируем две переменные экземпляра:
  # массив букв загаданного слова и пустой массив для сбора в него вводимых букв.
  def initialize(word)
    @letters = word.chars
    @user_guesses = []
  end

  # Возвращает массив букв, введенных пользователем, но отсутствующих в
  # загаданном слове (ошибочные буквы).
  def errors
    @user_guesses - normalized_letters
  end

  # Возвращает количество ошибок, сделанных пользователем.
  def errors_made
    errors.length
  end

  # Отнимает от допустимого количества ошибок число сделанных ошибок и
  # возвращает оставшееся число допустимых ошибок.
  def errors_allowed
    TOTAL_ERRORS_ALLOWED - errors_made
  end

  # Возвращает массив с уже отгаданными буквами, вместо неотгаданных букв
  # вставляется nil. Этот массив нужен для вывода слова на игровом табло.
  def letters_to_guess
    @letters.map { |letter| @user_guesses.include?(normalize_letter(letter)) ? letter : nil }
  end

  # Переводит буквы Ё/Й в Е/И.
  def normalize_letter(letter)
    case letter
    when "Ё" then "Е"
    when "Й" then "И"
    else letter
    end
  end

  # Возвращает загаданное слово с Е/И вместо Ё/Й.
  def normalized_letters
    @letters.map { |letter| normalize_letter(letter) }
  end

  # Возвращает true, если у пользователя не осталось попыток, т.е. игра проиграна.
  def lost?
    errors_allowed == 0
  end

  # Возвращает true, если игра закончена (проиграна или выиграна).
  def over?
    won? || lost?
  end

  # Основной игровой метод.Если игра не закончена и передаваемая буква
  # отсутствует в массиве введённых букв,
  # то вставляет передаваемую букву в массив попыток.
  def play!(letter)
    if !over? && !@user_guesses.include?(normalize_letter(letter))
      @user_guesses << normalize_letter(letter)
    end
  end

  # Возвращает true, если не осталось неотгаданных букв (пользователь выиграл).
  def won?
    (normalized_letters - @user_guesses).empty?
  end

  # Возвращает загаданное слово, склеивая его из загаданных букв.
  def word
    @letters.join
  end
end
